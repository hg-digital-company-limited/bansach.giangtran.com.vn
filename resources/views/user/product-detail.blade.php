                ...
                <div class="row" id="book" data-book-id="{{$book->BookID}}">
                    <div class="left-columm col-lg-5 col-md-5">
                        <div class="product-large-image tab-content">
                            ...
                            @if(isset($images) && count($images) > 0) <!-- Kiểm tra xem $images có tồn tại và không rỗng -->
                            @foreach ($images as $image)
                                <div class="tab-pane" id="product-0{{ $i }}" role="tabpanel" aria-labelledby="product-tab-0{{ $i++ }}">
                                    <div class="single-img img-full">
                                        <a href="{{$image->ImagePath}}">
                                            <img src="{{$image->ImagePath}}" class="img-fluid" alt="">
                                        </a>
                                    </div>
                                </div>
                            @endforeach
                            @else
                                <p>Không có hình ảnh nào để hiển thị.</p> <!-- Thông báo nếu không có hình ảnh -->
                            @endif
                        </div>

                        @if(isset($images) && count($images) > 0) <!-- Kiểm tra số lượng hình ảnh trước khi hiển thị danh sách hình nhỏ -->
                    ...
                    <div class="right-columm col-lg-7 col-md-7">
                        <div class="product-information">
                            <h4 class="product-title text-capitalize float-left w-100">
                                <a href="#" class="float-left w-100">{{ $book->BookTitle }}</a>
                            </h4>
            ...
            <div class="tab-content float-left w-100">
                ...
                <div class="tab-pane" id="product-tab2" role="tabpanel" aria-labelledby="tab2">
                    <div class="reviews-tab  float-left w-100" id="reviews-container">
                            ...
                            @foreach($reviews as $review)
                                <div class="ttreview-tab float-left w-100 p-30">
                                    ...
                                    <div class="review-title float-left w-100"><span
                                            class="date"> {{$review->created_at}}</span></div>
                        ...
                        @if($isPurchased)
                            <form action="{{ route('review.store') }}" method="POST" id="reviewForm" class="rating-form float-left w-100">
                                @csrf
    ...
    document.querySelector("#addToCart").addEventListener('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                // You can add some response handling here, like showing a success message
            },
...
function addReview() {
    ...
    stars.forEach(function (star) {
        star.addEventListener('click', function () {
            // Add 'selected' to the clicked star
    ...
    document.getElementById('reviewForm').addEventListener('submit', function (event) {
        ...
        fetch('/api/review', {
            ...
            .then(data => {
                ...
                deleteReview();  // Reinitialize deleteReview function

                // Reset the review form and stars
                resetReviewForm();
...
function newReviewHTMLCode(data) {
    ...
    return `<div class="ttreview-tab float-left w-100 p-30">
                ...
                <div class="review-title float-left w-100">
                    <span class="date"> ${data.FirstNamea}</span>
                </div>
                <div class="review-desc float-left w-100">${data.Content}</div>
                <div class="delete-button float-right">
                    <button type="button" class="btn btn-danger delete-comment" data-review-id="${data.ReviewID}">Xóa</button>
                </div>
            </div>`;
}
